...
class AuthAndStoryRegressionTestCase(unittest.TestCase):
    ...
    def test_login_by_sessionid_falls_back_after_empty_467_response(self):
        client = Client()
        sessionid = "1234567890123456789012345678901%3Atoken"
        response = Mock(status_code=467)
        client.user_info_v1 = Mock(side_effect=ClientError(response=response))
        client.user_stream_by_id_flat = Mock(return_value={"pk": "1234567890123456789", "username": "example"})
        client.user_short_gql = Mock(side_effect=AssertionError("sessionid login should use private fallback"))

        self.assertTrue(client.login_by_sessionid(sessionid))

        client.user_stream_by_id_flat.assert_called_once_with("1234567890123456789012345678901")
        client.user_short_gql.assert_not_called()
        self.assertEqual(client.username, "example")

    def test_login_by_sessionid_does_not_hide_other_http_errors(self):
        client = Client()
        sessionid = "1234567890123456789012345678901%3Atoken"
        response = Mock(status_code=500)
        error = ClientError(response=response)
        client.user_info_v1 = Mock(side_effect=error)
